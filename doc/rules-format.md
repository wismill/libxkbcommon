...
characters in the range `!` – `~`, excluding backslash `\`, comma `,` and
parentheses `()`. The first character shall not be one of `!`, `=`, `$`, `*`,
or `<`.
</dd>
<dt>`<kccgst-ident>`</dt>
<dd>
Denotes a lexical identifier: a non-empty maximal sequence of printable ASCII
characters in the range `!` – `~`, excluding backslash `\`, merge mode
characters `+|^` and qualifier prefix `:`. The first character shall not be
one of `!`, `=`, `$`, `*`, or `<`.
</dd>
</dl>
...
@important In the following grammar, whitespace is significant and shall not
appear unless explicitly shown.
