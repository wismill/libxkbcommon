...
libxkbcommon supports *effective* keyboard overlays since version 1.14.0. The
...
<td>
**No** overlap: each key can be part of *at most* one overlay.
</td>
<td>
A key may be assigned to **any overlay without restriction**. There is no limitation
on which overlay a given key may belong to, and a key may be assigned to *more
than one overlay*.
