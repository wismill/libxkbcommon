...
/* Ensure there is no implicit padding */
static_assert(sizeof(struct xkb_machine_builder_a11y_update_v1) ==
              sizeof(uint32_t)                      /* size   */
            + sizeof(uint32_t)                      /* affect */
            + sizeof(uint32_t),                     /* flags  */
              "struct xkb_machine_builder_a11y_update_v1 has implicit padding");
...
static_assert(sizeof(struct xkb_machine_builder_a11y_update_v1) ==
              sizeof(uint32_t)                      /* size   */
            + sizeof(xkb_mod_mask_t)                /* source */
            + sizeof(xkb_mod_mask_t),               /* target  */
              "struct xkb_machine_builder_mods_remap_update_v1 "
              "has implicit padding");
 ...
 * @since 1.14.0
 */
struct xkb_machine_builder_shortcut_mods_update_v1 {
    uint32_t size;
    xkb_mod_mask_t affect;
    xkb_mod_mask_t flags;
};

/* Ensure there is no implicit padding */
static_assert(sizeof(struct xkb_machine_builder_shortcut_mods_update_v1) ==
              sizeof(uint32_t)                      /* size   */
            + sizeof(xkb_mod_mask_t)                /* affect */
            + sizeof(xkb_mod_mask_t),               /* flags  */
              "struct xkb_machine_builder_shortcut_mods_update_v1 has implicit padding");

/* Current version is 1 */
static_assert(sizeof(struct xkb_machine_builder_shortcut_mods_update) ==
              sizeof(struct xkb_machine_builder_shortcut_mods_update_v1), "");

/* Ensure reasonable margin to the upper size limit */
static_assert(sizeof(struct xkb_machine_builder_shortcut_mods_update) * 30 <=
              (size_t)XKB_ABI_MAX_SIZE, "");
...
static_assert(
    xkb_versioned_struct_size_v1(((const struct xkb_machine_builder_shortcut_mods_update *)NULL)) <=
    xkb_versioned_struct_size_min(((const struct xkb_machine_builder_shortcut_mods_update *)NULL)),
    ""
);
...
static_assert(
    xkb_versioned_struct_size_min(((const struct xkb_machine_builder_shortcut_mods_update *)NULL)) <=
    sizeof(const struct xkb_machine_builder_shortcut_mods_update),
    ""
);
