...
HandleIncludeCompatMap(CompatInfo *info, IncludeStmt *include)
{
    ...
    InitCompatInfo(&included, info->ctx, info->include_depth + 1,
                   info->format, info->strict, &info->mods);
    ...
    for (IncludeStmt *stmt = include; stmt; stmt = stmt->next_incl) {
        ...
        InitCompatInfo(&next_incl, info->ctx, info->include_depth + 1,
                       info->format, info->strict, &included.mods);
    ...
    if (istreq(field, "action")) {
        ...
        if (value->common.type == STMT_EXPR_ACTION_LIST) {
            ...
            for (ExprDef *act = value->actions.actions;
                ...
                if (!HandleActionDef(info->ctx, info->format, info->strict,
        ...
        else if (HandleActionDef(info->ctx, info->format, info->strict,
                                 &info->default_actions, &info->mods,
               ...
               ExprDef *arrayNdx, ExprDef *value)
{
    ...
    if (istreq(field, "modifiers") || istreq(field, "mods")) {
        ...
        const xkb_layout_index_t max_groups =
            format_max_groups(info->format, info->strict);
...
HandleGlobalVar(CompatInfo *info, VarDef *stmt)
{
    ...
    if (!ExprResolveLhs(info->ctx, stmt->name, &elem, &field, &ndx))
        ...
        ret = SetDefaultActionField(info->ctx, info->format, info->strict,
                                    &info->default_actions, &info->mods,
                                    elem, field, ndx,
...
CompileCompatMap(XkbFile *file, struct xkb_keymap *keymap)
{
    ...
    InitCompatInfo(&info, keymap->ctx, 0, keymap->format,
                   (keymap->flags & XKB_KEYMAP_COMPILE_STRICT), &keymap->mods);
